...
test.describe('real-api ham', () => {
  test.skip(!isRealApi, 'Runs only when E2E_REAL_API=1.');

  test('transfer page loads with real API and no 5xx', async ({ page }) => {
    await setTestAuth(page, ['admin']);
    const statuses = captureStatuses(page, createApiPathMatcher('/ham/'));

    await page.goto('/ham/transfers', { waitUntil: 'domcontentloaded' });
    await expect(page).not.toHaveURL(/\/login(\?|$)/);

    await page.waitForTimeout(1500);
    assertObserved(statuses, 'ham API');
    assertNoServerErrors(statuses, 'ham API');
  });
});
