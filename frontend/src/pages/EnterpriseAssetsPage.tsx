...
export function EnterpriseAssetsPage() {
  ...
  const fetchAssets = useCallback(async () => {
    try {
      ...
      setError('Failed to load enterprise assets. Please try again.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (!isLoading && previousCountRef.current !== totalItems) {
      const message = totalItems === 0
        ? 'No enterprise assets match the current filters'
  ...
  if (error) {
    return (
      <PageLayout title="Enterprise Assets" description="Manage non-IT assets" breadcrumbs={breadcrumbs} maxWidth="xl">
        <ErrorMessage
          title="Error Loading Enterprise Assets"
          message={error}
          type="error"
          variant="inline"
  ...
  return (
    <PageLayout
      ...
      description="Manage non-IT assets including facilities equipment and machinery"
    ...
    >
      ...
      {!isLoading && assets.length === 0 ? (
        <EmptyState
          title="No enterprise assets found"
          description={hasActiveFilters ? 'Try adjusting your filters.' : 'Create your first enterprise asset to get started.'}
        ...
        <>
          <section aria-label="Enterprise assets list">
...
export default EnterpriseAssetsPage;
