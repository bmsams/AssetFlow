/**
 * Complete Disposal Lambda Handler
 *
 * Completes disposal and generates destruction certificate.
...
import type { CompleteDisposalRequest, DisposalMethod } from '../retirement/retirement-service';
import * as retirementService from '../retirement/retirement-service';

const logger = createLogger({ service: 'complete-disposal-handler' });
 ...
 */
function validateRequest(body: unknown): { valid: true; data: CompleteDisposalRequest } | { valid: false; errors: string[] } {
...
/**
 * Lambda handler for completing disposal
 */
export async function handler(
  ...
  logger.info('Complete disposal request received', { requestId, workflowId });
  ...
  try {
    ...
    const result = await retirementService.completeDisposal(
      workflowId,
      validation.data,
      userId
    );
    ...
    logger.error('Failed to complete disposal', err, { requestId, workflowId });
    ...
    if (err.message.includes('Cannot complete disposal')) {
      return createLambdaResponse(
    ...
    return createLambdaResponse(
      ...
      createErrorResponse(API_ERROR_CODES.INTERNAL_ERROR, 'Failed to complete disposal', requestId)
    );
  }
}
