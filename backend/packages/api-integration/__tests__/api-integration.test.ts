...
describe('Authentication Integration', () => {
  ...
  describe('JWT Token Validation', () => {
    it('should authenticate valid JWT token and extract user claims', async () => {
      ...
      expect(context.user.roles).toContain(ROLES.ASSET_MANAGER);
    });
  ...
  describe('Role-Based Access Control', () => {
    it('should allow ADMIN role to access all endpoints', async () => {
      ...
      const wrappedHandler = withAuth(mockVerifier, {
        requiredPermissions: [PERMISSIONS.ASSET_DELETE],
      })(handler);
    ...
    it('should allow ASSET_MANAGER to perform asset CRUD operations', async () => {
      ...
      const wrappedHandler = withAuth(mockVerifier, {
        requiredPermissions: [PERMISSIONS.ASSET_CREATE],
      })(handler);
    ...
    it('should deny VIEWER role from modifying assets', async () => {
      ...
      const wrappedHandler = withAuth(mockVerifier, {
        requiredPermissions: [PERMISSIONS.ASSET_UPDATE],
      })(handler);
    ...
    it('should allow INVENTORY_MANAGER to update inventory', async () => {
      ...
      const wrappedHandler = withAuth(mockVerifier, {
        requiredPermissions: [PERMISSIONS.INVENTORY_UPDATE],
      })(handler);
    ...
    it('should deny INVENTORY_MANAGER from managing software', async () => {
      ...
      const wrappedHandler = withAuth(mockVerifier, {
        requiredPermissions: [PERMISSIONS.SOFTWARE_MANAGE],
      })(handler);
...
describe('Request Validation Integration', () => {
  describe('Body Validation', () => {
    const assetCreateSchema: JsonSchema = {
      type: 'object',
    ...
    it('should accept valid asset creation request', async () => {
      ...
      const wrappedHandler = withValidation({ body: assetCreateSchema })(handler);
      const result = await wrappedHandler(event);
    ...
    it('should reject request with missing required fields', async () => {
      ...
      expect(body.error.details.map((d: { field: string }) => d.field)).toContain('displayName');
      expect(body.error.details.map((d: { field: string }) => d.field)).toContain('assetType');
    });
  ...
  describe('Path Parameter Validation', () => {
    ...
    it('should accept valid UUID path parameter', async () => {
      ...
      const wrappedHandler = withValidation({ pathParameters: pathSchema })(handler);
      const result = await wrappedHandler(event);
  ...
  describe('Query Parameter Validation', () => {
    ...
    it('should accept valid query parameters', async () => {
      ...
      const wrappedHandler = withValidation({ queryStringParameters: querySchema })(handler);
      const result = await wrappedHandler(event);
...
describe('Error Response Integration', () => {
  describe('Standardized Error Format', () => {
    it('should return standardized error response for validation errors', async () => {
      const schema: EndpointValidationSchema = {
        body: {
          type: 'object',
          required: ['name'],
    ...
    it('should include validation error details with field and code', async () => {
      ...
      body.error.details.forEach((detail: { field: string; message: string; code: string }) => {
...
describe('Pagination Integration', () => {
  const paginationOptions: PaginationMiddlewareOptions = {
    defaultLimit: 20,
    maxLimit: 100,
  ...
  describe('Page-Based Pagination', () => {
    it('should parse valid page and limit parameters', () => {
      ...
      const result = parseListQueryParams(event.queryStringParameters, paginationOptions);
  ...
  describe('Pagination Middleware', () => {
    it('should pass parsed pagination to handler', async () => {
      ...
      const wrappedHandler = withPagination(paginationOptions)(handler);
...
describe('Request Tracing Integration', () => {
  describe('Request ID Propagation', () => {
    it('should include request ID in successful responses', async () => {
      ...
      const handler = jest.fn().mockImplementation((_event, authContext) => {
        return createLambdaResponse(
          HTTP_STATUS.OK,
          createApiResponse({ message: 'success' }, authContext.requestId)
        );
      });
    ...
    it('should include CORS headers in responses', async () => {
      const response = createLambdaResponse(HTTP_STATUS.OK, { data: 'test' });
...
describe('Middleware Chain Integration', () => {
  ...
  describe('Combined Auth + Validation + Pagination', () => {
    ...
    it('should process request through complete middleware chain', async () => {
      ...
      let receivedAuthContext: AuthContext | null = null;
      ...
      const baseHandler = jest.fn().mockImplementation((_event, authContext) => {
        ...
        return createLambdaResponse(
          HTTP_STATUS.CREATED,
          createApiResponse({ id: 'new-asset-123' }, authContext.requestId)
        );
      });
      ...
      const validatedHandler = withValidation({ body: bodySchema })(
        async (event) => {
          ...
          const authHandler = withAuth(mockVerifier, {
            requiredPermissions: [PERMISSIONS.ASSET_CREATE],
          })(baseHandler);
      ...
      expect(receivedAuthContext!.user.roles).toContain(ROLES.ASSET_MANAGER);
    });
    ...
    it('should fail at validation before reaching auth', async () => {
      ...
      const validatedHandler = withValidation({ body: bodySchema })(baseHandler);
  ...
  describe('RESTful Endpoint Patterns', () => {
    ...
    it('should handle GET /assets (list) with pagination', async () => {
      ...
      const handler = jest.fn().mockImplementation((_event, authContext) => {
        return createLambdaResponse(HTTP_STATUS.OK, {
          data: [],
    ...
    it('should handle GET /assets/:id (single resource)', async () => {
      ...
      const validatedHandler = withValidation({ pathParameters: pathSchema })(
        async (event) => {
          const authHandler = withAuth(mockVerifier, {
            requiredPermissions: [PERMISSIONS.ASSET_READ],
          })(handler);
    ...
    it('should handle POST /assets (create)', async () => {
      ...
      const handler = jest.fn().mockImplementation((_event, authContext) => {
        return createLambdaResponse(HTTP_STATUS.CREATED, {
